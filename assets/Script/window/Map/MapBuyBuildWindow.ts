...
export default class MapBuyBuildWindow extends UIWindow {
    public static windowPath = 'Map/MapBuyBuildWindow';
    ...
    showBuildNode() {
        ...
        if (!this.mNode) {
            console.log('MapBuyBuildWindow: no map node');
            return;
        }
    ...
    onLevelUp() {
        ...
        if (Game.MergeTutorialManager && Game.MergeTutorialManager.EmitNodeClick) {
            Game.MergeTutorialManager.EmitNodeClick('building_buy_button', {
                mapId: this.mapID,

...
@ccclass('PassPortShopWindow')
export default class PassPortShopWindow extends UIWindow {
    public static windowPath = 'Activity/passport/PassPortShopWindow';
    ...
    onShow(showParams: any) {
        let metas = Meta.PassPortShopMeta.GetMetasByType(Meta.PassPortShopMeta.Type.Level);
        this.buy_item_list = [];
        for (let i = 0; i < this.items.length; i += 1) {
            ...
            let data = {
                ...
                label_buy_cost: GameKit.ControllerTable.GetNode(n, 'label_buy_cost').getComponent(Label),
                m: m,
            };
            let shopID = m.ShopID();
    ...
    callBuy(e: any, index: any) {
        ...
        AppKit.PaymentWrap.Pay(shopmeta.Name(), function(ok: any, res: any) {
            if (ok) {
